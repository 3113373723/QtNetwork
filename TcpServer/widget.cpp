...
void Widget::initServer()
{
    ...
    connect(ui->btnListen, &QPushButton::clicked, [this]{
        ...
        if(server->isListening()){
            ...
            //从界面读取ip和端口
            //可以使用QHostAddress::Any 监听所有地址的对应端口
    ...
    connect(server,&QTcpServer::newConnection,this,[this]{
        ...
        while(server->hasPendingConnections()){
            ...
            //套接字是作为服务器的子级创建的，这意味着销毁QTcpServer对象会自动删除该套接字
            //最好在完成处理后显式删除该对象，以避免浪费内存
            //返回的QTcpSocket对象不能从另一个线程使用，如有需要可重写incomingConnection()
            QTcpSocket *socket=server->nextPendingConnection();
            clientList.append(socket);
            ui->textRecv->appendPlainText(QString("[%1:%2] Socket Connected")
            ...
            //连接断开，销毁socket对象，确保开关server时socket正确释放
            connect(socket, &QTcpSocket::disconnected,[this, socket]{
                socket->deleteLater();
                clientList.removeOne(socket);
                ui->textRecv->appendPlainText(QString("[%1:%2] Socket Disconnected")
                                              .arg(socket->peerAddress().toString())
                                              .arg(socket->peerPort()));
                updateState();
            });
    ...
    //server的错误信息
        ...
        connect(server,&QTcpServer::acceptError,[this](QAbstractSocket::SocketError){
            ui->textRecv->appendPlainText(server->errorString());
        });
}
